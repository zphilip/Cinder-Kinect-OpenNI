...
xn::MockDepthGenerator g_MockDepth; // by Heresy, please see http://kheresy.wordpress.com/2012/05/01/modify-depth-map-for-user-generator/
...
void glutDisplay (void)
{
	...
	if (!g_bPause)
	{
		...
		// by Heresy, update depth data for mock node
		depthMD.MakeDataWritable();
		xn::DepthMap& rMap = depthMD.WritableDepthMap();
		for( unsigned int y = 0; y < rMap.YRes(); ++ y )
		{
			if( y % 5 == 0 )
			{
				for( unsigned int x = 0; x < rMap.XRes(); ++ x )
				{
					rMap( x, y ) = 0;
				}
			}
		}
		g_MockDepth.SetData(depthMD);
